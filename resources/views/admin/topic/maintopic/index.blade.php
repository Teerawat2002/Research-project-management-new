<x-app-layout>
    <div class="p-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div class="flex gap-2">
                <button type="button" data-modal-target="createMainTopicModal" data-modal-toggle="createMainTopicModal"
                    ...
                    <i class="fa-solid fa-plus mr-2"></i> เพิ่มหัวข้อหลัก
                </button>
            </div>
        </div>
        ...
        <div
            ...
            <div
                ...
                <form id="filterForm" method="GET" action="{{ route('admin.topic.maintopic.index') }}"
                    ...
                    <input type="hidden" name="form_id" id="hidden_form_id" value="{{ request('form_id') }}">
                    ...
                    <!-- Dropdown ตัวกรองแบบฟอร์ม -->
                    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        @php
                            $selectedFormName = 'หัวข้อทั้งหมด';
                            if (request('form_id') === 'unattached') {
                                $selectedFormName = 'ไม่อยู่ในแบบฟอร์ม';
                            } else {
                                foreach ($forms as $f) {
                                    if (request('form_id') == $f->id) {
                                        $selectedFormName = $f->name;
                                        break;
                                    }
                                }
                            }
                            ...
                            <x-dropdown align="right" width="56">
                                <x-slot name="trigger">
                                    <button type="button"
                                        ...
                                        <span class="truncate">{{ $selectedFormName }}</span>
                                        <i
                                ...
                                <x-slot name="content">
                                    <x-dropdown-link href="#"
                                        onclick="event.preventDefault(); document.getElementById('hidden_form_id').value=''; document.getElementById('filterForm').submit();"
                                        class="{{ request('form_id') == '' ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                        หัวข้อทั้งหมด
                                    </x-dropdown-link>

                                    <x-dropdown-link href="#"
                                        onclick="event.preventDefault(); document.getElementById('hidden_form_id').value='unattached'; document.getElementById('filterForm').submit();"
                                        class="{{ request('form_id') == 'unattached' ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                        ไม่อยู่ในแบบฟอร์ม
                                    </x-dropdown-link>
                                    ...
                                    @foreach ($forms as $f)
                                        <x-dropdown-link href="#"
                                            onclick="event.preventDefault(); document.getElementById('hidden_form_id').value='{{ $f->id }}'; document.getElementById('filterForm').submit();"
                                            class="{{ request('form_id') == $f->id ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                            {{ $f->name }}
                                        </x-dropdown-link>
                                    @endforeach
                                </x-slot>
                ...
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400 transition-colors duration-200">
                    <thead
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 font-medium">ชื่อหัวข้อ</th>
                            <th scope="col" class="px-6 py-4 font-medium w-32">คะแนน</th>
                            <th scope="col" class="px-6 py-4 font-medium truncate">แบบฟอร์ม</th>
                            ...
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700 transition-colors duration-200 group">
                                ...
                                <td class="px-6 py-4 text-right space-x-3 text-gray-400 dark:text-gray-500">
                                    <button type="button" title="แก้ไข"
                                        class="hover:text-blue-500 dark:hover:text-blue-400 transition-colors btn-edit-maintopic"
                                        data-modal-target="editMainTopicModal" data-modal-toggle="editMainTopicModal"
    ...
    <!-- Modal Create -->
    <div id="createMainTopicModal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl transition-colors duration-200">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white">เพิ่มหัวข้อหลัก</h3>
                    </div>
                    <button type="button"
                        ...
                        data-modal-hide="createMainTopicModal">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                @include('admin.topic.maintopic.partials.create-form')
            </div>
        </div>
    </div>

    <!-- Modal Edit -->
    <div id="editMainTopicModal" tabindex="-1" aria-hidden="true"
        class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto inset-0 h-modal h-full bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-300">
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl transition-colors duration-200">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white">แก้ไขหัวข้อหลัก</h3>
                    </div>
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:text-gray-900 dark:hover:text-white rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                        data-modal-hide="editMainTopicModal">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                @include('admin.topic.maintopic.partials.edit-form')
            </div>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                const editButtons = document.querySelectorAll('.btn-edit-maintopic');
                const editForm = document.getElementById('editMainTopicForm');
                const updateBaseUrl =
                "{{ url('admin/topic/maintopic') }}"; // ตรวจสอบ Route ให้ตรงกับ web.php ของคุณ
