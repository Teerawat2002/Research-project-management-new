<form id="createSubTopicForm" method="POST" action="{{ route('admin.topic.subtopic.store') }}"
    ...
    <div class="mb-5">
        <label
            ...
            เลือกหัวข้อหลัก <span class="text-red-500 dark:text-red-400">*</span>
        </label>

        <input type="hidden" name="mtopic_id" id="create_mtopic_id" value="{{ old('mtopic_id') }}" required>
        ...
        <div class="flex shadow-sm rounded-xl w-full">
            ...
            <!-- ใส่ flex-1 min-w-0 บังคับไม่ให้กล่องยืดทะลุจอ -->
            <div class="relative flex-1 min-w-0">
                <button type="button" id="createDropdownBtn" data-dropdown-toggle="createDropdownMenu"
                    ...
                    <!-- ใส่ flex-1 text-left truncate ให้ข้อความตัดคำอัตโนมัติ และผลักลูกศรไปขวาสุด -->
                    <span id="createDropdownText" class="truncate flex-1 text-left pr-2">เลือกหัวข้อหลัก</span>
                    <!-- ใส่ shrink-0 ป้องกันลูกศรโดนบีบหายไป -->
                ...
                <div id="createDropdownMenu"
                    ...
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="createDropdownBtn">
                        @foreach ($main_topics as $main_topic)
                            <li>
                                <a href="#"
                                    onclick="event.preventDefault(); document.getElementById('create_mtopic_id').value='{{ $main_topic->id }}'; document.getElementById('createDropdownText').innerText='{{ addslashes($main_topic->name) }}'; document.getElementById('createDropdownBtn').click();"
                                    ...
                                    {{ \Illuminate\Support\Str::limit($main_topic->name, 60, '...') }}
                                </a>
                            </li>
                        @endforeach
                    </ul>
    ...
    <div class="mb-5">
        <label for="name"
            class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5 transition-colors duration-200">
            ชื่อหัวข้อรอง <span class="text-red-500 dark:text-red-400">*</span>
        </label>
        <div class="flex shadow-sm rounded-xl w-full">
            <span
                class="inline-flex shrink-0 items-center px-4 text-sm text-gray-500 bg-gray-100 border border-e-0 border-gray-200 rounded-s-xl dark:bg-gray-700 dark:text-gray-400 dark:border-gray-600 transition-colors duration-200">
            ...
            <input type="text" id="name" name="name" required
                ...
                placeholder="เช่น ส่วนหน้าบ้าน (Frontend)" value="{{ old('name') }}">
        </div>
        @error('name')
            <p class="text-red-500 dark:text-red-400 text-sm mt-1.5">{{ $message }}</p>
        @enderror
    ...
    <div class="mb-6">
        ...
        <div class="flex shadow-sm rounded-xl w-full">
            ...
            <input type="number" id="score" name="score" required min="0" step="0.5"
                class="rounded-none rounded-e-xl block flex-1 min-w-0 w-full px-4 py-2.5 text-sm text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 dark:focus:ring-orange-500 transition-colors duration-200"
                placeholder="เช่น 10" value="{{ old('score') }}">
        </div>
        @error('score')
            <p class="text-red-500 dark:text-red-400 text-sm mt-1.5">{{ $message }}</p>
        @enderror
    ...
    <div class="flex items-center justify-end gap-3">
        <button type="button" data-modal-hide="createSubTopicModal"
