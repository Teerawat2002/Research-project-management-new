<x-app-layout>
    <div class="p-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div class="flex gap-2">
                <button type="button" data-modal-target="createSubSubTopicModal"
                    data-modal-toggle="createSubSubTopicModal"
                    ...
                    <i class="fa-solid fa-plus mr-2"></i> เพิ่มหัวข้อย่อย
                </button>
            </div>
        </div>
        ...
        <div
            ...
            <div
                ...
                <form id="filterForm" method="GET" action="{{ route('admin.topic.subsubtopic.index') }}"
                    ...
                    <input type="hidden" name="subtopic_id" id="hidden_subtopic_id"
                        value="{{ request('subtopic_id') }}">
                    ...
                    <div class="relative w-full sm:w-80">
                        ...
                        <input type="text" name="search" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="ค้นหาหัวข้อย่อย... (กด Enter)">
                    </div>

                    <!-- Dropdown ตัวกรองหัวข้อรอง -->
                    <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                        @php
                            $selectedSubTopicName = 'หัวข้อรองทั้งหมด';
                            if (request('subtopic_id') === 'unattached') {
                                $selectedSubTopicName = 'ไม่อยู่ในหัวข้อรอง';
                            } else {
                                foreach ($subtopics as $st) {
                                    if (request('subtopic_id') == $st->id) {
                                        $selectedSubTopicName = $st->name;
                                        break;
                                    }
                                }
                            }
                            ...
                            <x-dropdown align="right" width="64">
                                <x-slot name="trigger">
                                    <button type="button"
                                        ...
                                        <span class="truncate">{{ $selectedSubTopicName }}</span>
                                        <i
                                ...
                                <x-slot name="content">
                                    <x-dropdown-link href="#"
                                        onclick="event.preventDefault(); document.getElementById('hidden_subtopic_id').value=''; document.getElementById('filterForm').submit();"
                                        class="{{ request('subtopic_id') == '' ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                        หัวข้อรองทั้งหมด
                                    </x-dropdown-link>

                                    <x-dropdown-link href="#"
                                        onclick="event.preventDefault(); document.getElementById('hidden_subtopic_id').value='unattached'; document.getElementById('filterForm').submit();"
                                        class="{{ request('subtopic_id') == 'unattached' ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}">
                                        ไม่อยู่ในหัวข้อรอง
                                    </x-dropdown-link>
                                    ...
                                    @foreach ($subtopics as $st)
                                        <x-dropdown-link href="#"
                                            onclick="event.preventDefault(); document.getElementById('hidden_subtopic_id').value='{{ $st->id }}'; document.getElementById('filterForm').submit();"
                                            class="{{ request('subtopic_id') == $st->id ? 'bg-orange-50 text-orange-600 font-bold dark:bg-gray-700 dark:text-orange-400' : 'text-gray-700 dark:text-gray-200 dark:hover:bg-gray-700 dark:hover:text-white' }}"
                                            title="{{ $st->name }}">
                                            {{ \Illuminate\Support\Str::limit($st->name, 40, '...') }}
                                        </x-dropdown-link>
                                    @endforeach
                                </x-slot>
                            </x-dropdown>
                ...
                <table class="w-full text-sm text-left text-gray-600 dark:text-gray-400 transition-colors duration-200">
                    <thead
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 font-medium">ชื่อหัวข้อย่อย</th>
                            <th scope="col" class="px-6 py-4 font-medium truncate">หัวข้อรอง</th>
                            ...
                            <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-700 transition-colors duration-200 group">
                                ...
                                <td class="px-6 py-4 text-right space-x-3 text-gray-400 dark:text-gray-500">
                                    <button type="button" title="แก้ไข"
                                        class="hover:text-blue-500 dark:hover:text-blue-400 transition-colors btn-edit-subsubtopic"
                                        data-modal-target="editSubSubTopicModal"
                                        data-modal-toggle="editSubSubTopicModal" data-id="{{ $topic->id }}"
                                        data-name="{{ $topic->name }}" data-stopic_id="{{ $topic->stopic_id }}"
    ...
    <!-- Modal Create -->
    <div id="createSubSubTopicModal" tabindex="-1" aria-hidden="true"
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl transition-colors duration-200">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white">เพิ่มหัวข้อย่อย</h3>
                    </div>
                    <button type="button"
                        ...
                        data-modal-hide="createSubSubTopicModal">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                @include('admin.topic.subsubtopic.partials.create-form')
            </div>
        </div>
    </div>

    <!-- Modal Edit -->
    <div id="editSubSubTopicModal" tabindex="-1" aria-hidden="true"
        class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto inset-0 h-modal h-full bg-black bg-opacity-50 flex items-center justify-center transition-opacity duration-300">
        <div class="relative w-full max-w-md max-h-full">
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl transition-colors duration-200">
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-lg font-bold text-gray-800 dark:text-white">แก้ไขหัวข้อย่อย</h3>
                    </div>
                    <button type="button"
                        class="text-gray-400 bg-transparent hover:text-gray-900 dark:hover:text-white rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                        data-modal-hide="editSubSubTopicModal">
                        <i class="fa-solid fa-xmark text-lg"></i>
                    </button>
                </div>
                @include('admin.topic.subsubtopic.partials.edit-form')
            </div>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                const editButtons = document.querySelectorAll('.btn-edit-subsubtopic');
                const editForm = document.getElementById('editSubSubTopicForm');
                const updateBaseUrl = "{{ url('admin/topic/subsubtopic') }}"; // ตรวจสอบ Route ให้ตรงกับ web.php ของคุณ

                // ดึงรายชื่อ Sub Topic มาทำ Mapping เพื่อใช้แปลง ID เป็นชื่อบนปุ่ม Dropdown
                const subTopicMap = {
                    @foreach ($subtopics as $st)
                        "{{ $st->id }}": "{{ addslashes($st->name) }}",
                    @endforeach
                };
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const stopicId = this.getAttribute('data-stopic_id');

                        document.getElementById('edit_name').value = this.getAttribute('data-name');
                        document.getElementById('edit_score').value = this.getAttribute('data-score');
                        document.getElementById('edit_stopic_id').value = stopicId;

                        // เปลี่ยนข้อความบนปุ่ม Dropdown ให้ตรงกับหัวข้อที่ถูกเลือก
                        if (stopicId && subTopicMap[stopicId]) {
                            let text = subTopicMap[stopicId];
                            if (text.length > 60) text = text.substring(0, 60) + '...';
                            document.getElementById('editDropdownText').innerText = text;
                        } else {
                            document.getElementById('editDropdownText').innerText = 'เลือกหัวข้อรอง';
                        }
                ...
                document.querySelectorAll('.delete-button').forEach(function(button) {
                    button.addEventListener('click', function(e) {
                        ...
                        Swal.fire({
                            title: 'คุณต้องการลบหัวข้อย่อย "' + topicName + '" ใช่หรือไม่?',
