<form id="createSubSubTopicForm" method="POST" action="{{ route('admin.topic.subsubtopic.store') }}"
    ...
    <div class="mb-5">
        ...
        <input type="hidden" name="stopic_id" id="create_stopic_id" value="{{ old('stopic_id') }}" required>
        ...
        <div class="flex shadow-sm rounded-xl w-full">
            ...
            <div class="relative flex-1 min-w-0">
                <button type="button" id="createDropdownBtn" data-dropdown-toggle="createDropdownMenu"
                    ...
                    <span id="createDropdownText" class="truncate flex-1 text-left pr-2">เลือกหัวข้อรอง</span>
                ...
                <div id="createDropdownMenu"
                    ...
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="createDropdownBtn">
                        @foreach ($subtopics as $st)
                            <li>
                                <a href="#"
                                    onclick="event.preventDefault(); document.getElementById('create_stopic_id').value='{{ $st->id }}'; document.getElementById('createDropdownText').innerText='{{ addslashes($st->name) }}'; document.getElementById('createDropdownBtn').click();"
    ...
    <div class="mb-5">
        ...
        <div class="flex shadow-sm rounded-xl w-full">
            ...
            <input type="text" id="name" name="name" required
                ...
                placeholder="เช่น การจัดรูปแบบหน้าจอ" value="{{ old('name') }}">
        </div>
        @error('name')
            <p class="text-red-500 dark:text-red-400 text-sm mt-1.5">{{ $message }}</p>
        @enderror
    ...
    <div class="mb-6">
        ...
        <div class="flex shadow-sm rounded-xl w-full">
            ...
            <input type="number" id="score" name="score" required min="0" step="0.5"
                class="rounded-none rounded-e-xl block flex-1 min-w-0 w-full px-4 py-2.5 text-sm text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 focus:border-orange-500 focus:ring-2 focus:ring-orange-200 dark:focus:ring-orange-500 transition-colors duration-200"
                placeholder="เช่น 5" value="{{ old('score') }}">
        </div>
        @error('score')
            <p class="text-red-500 dark:text-red-400 text-sm mt-1.5">{{ $message }}</p>
        @enderror
    ...
    <div class="flex items-center justify-end gap-3">
        <button type="button" data-modal-hide="createSubSubTopicModal"
